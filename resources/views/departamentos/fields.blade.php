<form action="{{ route('departamentos.store') }}" method="POST">
    ...
    <div class="row">
        ...
        <div class="col-md-6 mb-3">
            ...
            <input type="text" name="encargado" id="encargado" class="form-control @error('encargado') is-invalid @enderror" value="{{ old('encargado') }}">
            <div class="invalid-feedback">
                @error('encargado')
                    {{ $message }}
                @enderror
        ...
        <div class="col-md-6 mb-3">
            ...
            <select name="ente_id" id="ente_id" class="form-select @error('ente_id') is-invalid @enderror" required>
                ...
                @foreach ($entes as $ente)
                    <option value="{{ $ente->id }}" {{ old('ente_id') == $ente->id ? 'selected' : '' }}>{{ $ente->nombre }}</option>
                @endforeach
            </select>
            <div class="invalid-feedback">
                @error('ente_id')
